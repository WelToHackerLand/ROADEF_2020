#ifndef NLSACO_SOLUTION_VER2
#define NLSACO_SOLUTION_VER2
    ...
    void process(Problem_Instance &instance, string outputFile, double timeLimit, double alpha, double beta) {
        ...
        for (int loop = 1; loop <= iLimit; ++loop) {
            ...
            for (int ant = 1; ant <= nAnts; ++ant) {
                ...
                /// recreate
                NLS_repair_procedure::process_version_2( instance, Erase_list, obj, alpha, beta );
                if ( obj.numFailedIntervention > 0 ) continue;
                if ( obj.LBResources_cost > 1e-5 || obj.UBResources_cost > 1e-5 ) continue;
