        ...
        for (int id = 0; id < (int) lsTime.size(); ++id) {
            ...
            for (int mask = (1<<numBit)-1; mask >= 0; --mask) {
                ...
                for (int bit = 0; bit < (int) numBit; ++bit) {
                    ...
                    tmpOBJ.Erase_Only_Resources(instance, tmpOBJ, inter, oldTime);
                    if ( !tmpOBJ.check_resource_constraint(instance, tmpOBJ, inter, t) ) {
                        tmpOBJ.Insert_Only_Resources(instance, tmpOBJ, inter, oldTime);
                        continue;
                    }
